...
5. **Physical-world consequences distinguishing from abstract data processing:** The system's output directly determines which physical healthcare facility a patient is transported to, which medical capabilities (blood bank, operation theatre, specialist) will be available upon arrival, and whether blood products should be prepared for a specific blood type. These are physical-world consequences — ambulance dispatch, facility pre-alerting, blood preparation — that distinguish the invention from abstract data processing. The system operates at the boundary of physical healthcare delivery: it processes physiological measurements (hemoglobin concentration in blood, arterial blood pressure, body mass) and geographic coordinates of real physical facilities, and its outputs route patients to specific named hospitals verified to have the required medical capabilities. This is analogous to the Niramai precedent where AI processing of physical thermal measurements produced clinical diagnostic output.
...
- The multiplicative risk model assumes conditional independence across dimensions, with interaction terms applied for known correlated factor pairs to partially mitigate this assumption. This is a deliberate design choice balancing precomputability against model fidelity: the 70,000-entry table (7 dimensions) remains tractable for precomputation and deployment on resource-constrained devices. Adding all pairwise interactions would multiply entries by C(7,2) × interaction levels, exceeding practical precomputation and storage limits for the target deployment environment (ASHA worker smartphones). The four selected interaction terms (anemia+PPH: 1.8×, HTN+eclampsia: 2.0×, adolescent+primigravida: 1.3×, anemia+hypertension: 1.5×) cover the combinations with highest published synergistic risk. The system is calibrated to be conservative (over-predict risk), and the Bayesian updater will correct systematic errors from independence assumptions as deployment outcome data accumulates.
- No prospective clinical validation has been conducted. The system is designed as decision-support pending field validation via clinical trial. Risk factor weights are derived from published peer-reviewed meta-analyses and systematic reviews (see Citations Index). The utility of the invention lies in producing integrated risk-aware referral recommendations in constant time — routing patients to specific facilities verified to have required medical capabilities based on temporally-aware risk classification. This utility can be validated by a person skilled in the art by comparing the system's outputs against expert obstetrician assessments for concordance. The individual relative risk values used are each independently validated in published literature; the multiplicative combination follows established epidemiological methodology (Rothman KJ, "Epidemiology: An Introduction", Oxford 2012).
...
**Claim Structure (revised):** Claim 1 (Method) — integrated pipeline with Hb feedback and capability-aware routing; Claim 2 (System) — stateless precomputed deployment; Claim 3 (dep.) — regional Bayesian calibration; Claim 4 (dep.) — learned index MLP architecture details; Claim 5 (dep.) — Count-Min Sketch blood bank estimation. Claims 5-7 from prior draft (pub-sub updates, offline Gemma, federated CMS) moved to Future Embodiments.
...
| 19 | Claim 1 included voice input/NLU steps | Voice interface disclaimed as non-inventive in specification, creating contradiction | Removed voice/NLU from Claim 1; claim now starts at "receiving structured maternal health parameters" | Eliminates prosecution risk from contradictory disclaimer |
| 20 | Claim 1 lacked Hb feedback loop | Core novelty (closed-loop integration) was in dependent claim, not independent | Moved Hb trajectory prediction and risk re-scoring into Claim 1 steps (e)-(f) | Independent claim now captures the inventive combination |
| 21 | Claim 2 said "real-time facility attributes" | Implementation uses static precomputed JSON, not real-time data | Changed to "facility attributes... precomputed from government health facility registries" | Claim matches implementation |
| 22 | Claims 5, 6, 7 described unimplemented features | Enablement failure under Section 10(4) — cannot teach POSITA to practice | Moved to "Future Embodiments" section in specification; removed from claims | Eliminates enablement objection |
| 23 | Facility count "30,284" in architecture doc | Only 10,602 had valid geocoordinates; corrections log #1 fixed questionnaire but not architecture doc | Changed to "10,602 geocoded facilities in the exemplary embodiment (28,128 total parsed)" throughout | Consistency across all documents |
| 24 | Key Components said "No neural network — deterministic physiological model" | Learned index MLP is a neural network (2,497 parameters) | Corrected to describe learned index MLP with fallback to analytical model | Eliminates internal contradiction |
| 25 | No pipeline comparison example | Examiner may question what "synergistic technical effect" means concretely | Added comparative example (Sunita case) showing Engine-alone vs. Integrated output differences | Demonstrates non-obvious pipeline benefit |
| 26 | Conditional independence acknowledged but not justified | Examiner may question reliability of risk scores | Added design rationale: tractability constraint, conservative calibration, Bayesian correction mechanism | Preempts examiner utility/reliability objection |
| 27 | No utility evidence for unvalidated system | "No prospective clinical validation" without utility argument | Added utility statement: system produces verifiable referral recommendations; individual RR values are independently validated; multiplicative methodology is established epidemiology | Preempts utility objection |
| 28 | Section 3(k) defense lacked physical-world output argument | Defense focused on input processing, not output consequences | Added point 5: ambulance dispatch, facility pre-alerting, blood preparation are physical-world consequences | Strengthens technical effect argument |
