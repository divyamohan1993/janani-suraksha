...
JananiSuraksha is an AI-powered maternal health risk intelligence platform that predicts high-risk pregnancies, optimizes emergency obstetric referral routing, and provides continuous antenatal monitoring through ASHA worker smartphones — addressing India's maternal mortality crisis where ~22,500 women die annually from preventable obstetric complications. India's Maternal Mortality Ratio (MMR) stands at 88 per 100,000 live births (SRS 2021-23), with the poorest states bearing catastrophic burden: Assam (195), Uttar Pradesh (167), Madhya Pradesh (153). The fundamental failure is the "three delays" — delay in deciding to seek care, delay in reaching a facility, and delay in receiving adequate treatment — which collectively account for most maternal deaths. Yet India has 1.18 million ASHA workers who visit every pregnant woman monthly, but they lack any AI tool to predict which pregnancies will turn dangerous. JananiSuraksha introduces three technical contributions integrated in a closed-loop pipeline: (1) **Constant-Time Maternal Risk Scoring via Precomputed Multiplicative Relative Risk Tables** that map 70,000 combinations of maternal risk factors (age, parity, hemoglobin, blood pressure, gestational week, BMI, complication history) to literature-calibrated risk scores retrievable via single hash-indexed table lookup, enabling ASHA workers to instantly classify pregnancies as low/medium/high/critical risk; (2) **Constant-Time Emergency Referral Routing via Precomputed Facility-Capability Spatial Index** that indexes India's health facility network (10,602 geocoded facilities in the exemplary embodiment from data.gov.in) with precomputed spatial nearest-neighbor lookup tables per capability level, enabling instant optimal facility recommendation given a mother's location and required care level determined by the risk assessment; and (3) **Constant-Time Anemia Progression Prediction via Learned Index on Hemoglobin Trajectories** that uses a learned index MLP (2,497 parameters) to predict hemoglobin trajectory from continuous clinical inputs, with predicted delivery hemoglobin feeding back to re-score risk — producing temporally-aware risk classification that accounts for projected anemia progression. The closed-loop integration is the core invention: predicted hemoglobin trajectory (Engine 3) feeds back to re-evaluate risk (Engine 1); risk level auto-selects facility capability via the WHO/UNFPA/UNICEF/AMDD EmOC framework; capability drives spatial routing (Engine 2). All intelligence is precomputed at build time — no GPU, database, or network dependency at runtime. Deployed at jananisuraksha.dmj.one.
...
- **Current implementation**: 10,602 geocoded facilities in the exemplary embodiment (28,128 total parsed from data.gov.in National Hospital Directory, resource ID: 37670b6f-c236-49a7-8cd7-cc2dc610e32d)
...
### Integrated Pipeline vs. Individual Engines — Comparative Example

To illustrate the synergistic technical effect of the closed-loop integration, consider the following patient:

**Patient:** Sunita, 32, parity 3, current Hb 9.5 g/dL, BP 125/80, 22 weeks, BMI 23, history of PPH. Nearest facility: PHC Laharpur (8 km, basic_emoc only). Nearest with blood bank: District Hospital Sitapur (28 km, comprehensive_emoc).

| Component | Output if Engine Operates Alone | Output from Integrated Pipeline |
|-----------|-------------------------------|-------------------------------|
| **Engine 1 only** (risk scoring with current Hb 9.5) | MEDIUM risk (score 0.04) — moderate anemia + PPH history, but current Hb above severe threshold | Initial: MEDIUM (0.04). After Hb feedback: **HIGH** (0.08) — because projected delivery Hb of 7.8 pushes risk upward |
| **Engine 3 only** (anemia prediction) | Predicted delivery Hb: 7.8 g/dL (moderate→severe trajectory). Alert: "anemia worsening" | Same prediction, but output **feeds back** to Engine 1 for re-scoring, which changes risk level from MEDIUM to HIGH |
| **Engine 2 only** (referral routing, if given "MEDIUM" risk) | Routes to PHC Laharpur (8 km, basic_emoc) — meets MEDIUM requirement | Routes to **District Hospital Sitapur (28 km, blood_transfusion)** — because re-scored HIGH risk requires blood_transfusion capability |
| **Integrated pipeline** | N/A | Patient routed to facility with blood bank 20 km further, but one that can manage the PPH recurrence that her declining Hb trajectory makes more likely. Count-Min Sketch estimates O+ availability at the routed facility. |

**Key insight:** Without the closed-loop integration, this patient is routed to a facility without blood bank capability. With integration, the predicted Hb decline triggers risk re-scoring, which escalates the required facility capability, which changes the referral destination. No individual engine produces this temporally-aware, capability-matched referral.
...
│ Tables         │ │ Spatial      │ │ Trajectories      │
│ 70K entries    │ │ Nearest-     │ │ 7,480 profiles    │
│ const lookup   │ │ Neighbor Idx │ │ const prediction  │
│                │ │ const routing│ │                   │
└──────┬─────────┘ └──────┬───────┘ └────────┬──────────┘
       │                  │                   │
       ▼                  ▼                   ▼
...
│  real_facilities.json (28,128 facilities from data.gov.in)  │
└─────────────────────────────────────────────────────────┘
       │                  │                   │
       ▼                  ▼                   ▼
┌─────────────────────────────────────────────────────────┐
...
3. **Facility-Capability Graph Engine**: Indexes 10,602 geocoded facilities in the exemplary embodiment (28,128 total parsed from data.gov.in, resource ID: 37670b6f-c236-49a7-8cd7-cc2dc610e32d), plus 2,484 blood banks with coordinates (resource ID: fced6df9-a360-4e08-8ca0-f283fc74ce15). Grid-based spatial index stored as JSON, loaded into memory at startup.

4. **Hemoglobin Trajectory Prediction via Learned Index**: 7,480 trajectory profiles generated from a literature-calibrated physiological model, sorted by predicted delivery hemoglobin. Primary lookup: a learned index MLP (5→64→32→1, 2,497 parameters, 68 KB) predicts approximate position in the sorted array from continuous clinical inputs; bounded local search (±20 positions) refines to exact match. Fallback chain: hash-based discretized index → analytical physiological model computation on cache miss. No runtime ML framework dependency — pure Python forward pass.
...
2. **Facility Graph**: Spatial index representation. 10,602 geocoded facilities in the exemplary embodiment (28,128 total parsed from data.gov.in) + 2,484 blood banks. Node attributes: {id, name, type, capabilities, specialist_status, geographic coordinates}. Grid-based index: 0.1-degree cells (~11 km) mapping to nearest facility per capability level. Total: ~106 MB as JSON (facility_graph.json).

3. **Spatial Nearest-Neighbor Index**: For each capability level (5 levels), a table mapping grid_cell → nearest_facility. 46,013 precomputed grid cells per capability level in the exemplary embodiment (0.1-degree resolution, ~11 km cells across India). Primary + backup (2nd-nearest) facility precomputed per cell.
...
1. **Claim 1 (Method)**: A computer-implemented method for integrated maternal risk assessment with temporally-aware referral routing, comprising: (a) receiving structured maternal health parameters comprising values in seven clinical dimensions including age, parity, hemoglobin concentration, blood pressure, gestational week, body mass index, and complication history; (b) discretizing the parameters into predetermined bucket indices across the seven clinical dimensions; (c) computing a hash key from the discretized indices; (d) retrieving a precomputed maternal risk score from a hash-indexed table of multiplicative relative risk model entries, wherein the table comprises 70,000 or more entries precomputed from literature-calibrated relative risk values derived from published epidemiological research; (e) predicting delivery hemoglobin concentration using a learned index structure on precomputed hemoglobin trajectory profiles, wherein the learned index accepts continuous clinical inputs and predicts an approximate position in a sorted trajectory array with bounded local search refining to exact match; (f) re-scoring the maternal risk using the predicted delivery hemoglobin when the projected value is lower than the current value, thereby producing a temporally-aware risk classification that accounts for projected anemia progression; (g) mapping the risk classification to a required facility capability level via an emergency obstetric care framework; and (h) routing to the nearest facility with the required capability via a precomputed spatial nearest-neighbor index — wherein steps (b) through (h) execute in constant or bounded-constant computational time with no runtime database, GPU, or network dependency for core computation.

2. **Claim 2 (System)**: A system for implementing the method of Claim 1, comprising: (a) a health facility database storing facility attributes including capability level, geographic location, and specialist status, precomputed from government health facility registries; (b) precomputed spatial nearest-neighbor lookup tables mapping geographic grid cells to facilities, computed per required capability level using haversine distance, with primary and backup facilities precomputed per cell; (c) a risk-to-capability mapping interface that translates risk classifications from the risk assessment engine into required facility capability levels according to an emergency obstetric care framework; (d) a query interface that, given a pregnant woman's geographic location and required capability level, retrieves the optimal referral destination via grid-key lookup in constant time; and (e) the system deployed as a stateless container with all intelligence precomputed at build time, operable on resource-constrained devices over intermittent networks.

### Dependent Claims

1. **Claim 3** (dependent on Claim 1): The method of Claim 1, wherein Bayesian posterior risk scores are maintained separately for each administrative region via conjugate Beta-Binomial updating from observed birth outcomes, enabling regional calibration of risk thresholds based on local maternal mortality patterns without model retraining.

2. **Claim 4** (dependent on Claim 1): The method of Claim 1, wherein the learned index structure of step (e) comprises a 2-layer neural network (5→64→32→1, 2,497 parameters) trained to approximate the cumulative distribution function of a sorted array of 7,480 hemoglobin trajectory profiles generated from a physiological model incorporating linear hemoglobin decline, Gaussian hemodilution, iron supplementation effect, and previous anemia recurrence risk. The learned index accepts continuous inputs (initial hemoglobin, gestational week, iron-folic acid compliance, dietary score, previous anemia history) and predicts the approximate position in the sorted trajectory array, with bounded local search (±20 positions) refining to exact match. A fallback chain provides hash-based discretized index lookup and analytical physiological model computation when the learned index is unavailable.

3. **Claim 5** (dependent on Claim 1): The method of Claim 1, wherein blood bank inventory levels at the routed facility are estimated using a Count-Min Sketch probabilistic data structure (width=256, depth=4), providing constant-time blood availability estimation across multiple blood types with bounded error, and the estimate is included in the integrated assessment response to inform clinical preparation at the receiving facility.

### Future Embodiments

The following capabilities are described for completeness and are planned for future implementation. They are not part of the current claims:

- **Incremental spatial index updates** via publish-subscribe messaging upon facility status changes, without requiring full recomputation.
- **Offline inference mode** using a knowledge-distilled small language model deployed on the health worker's mobile device for areas without network connectivity.
- **Federated Count-Min Sketch** across facilities for decentralized blood bank inventory aggregation without centralized real-time database synchronization.
